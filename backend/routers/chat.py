...
[언어 규칙] 오직 한글과 숫자, 기본 문장부호(.,!?)만 사용하세요. 한자, 히라가나, 가타카나, 키릴 문자, 한자, 알파벳 등 한글이 아닌 문자는 단 한 글자도 사용하지 마세요.
...
def get_goal_advice(goal_id: str, authorization: str = Header(...)):
    ...
    response = groq_client.chat.completions.create(
        ...
        messages=[
            {
                "role": "system",
                "content": """당신은 재무 목표 코치 AI입니다. [언어 규칙] 반드시 한국어로만 답변하세요. 영어, 일본어, 태국어, 중국어 등 어떤 외국어도 절대 사용하지 마세요.
